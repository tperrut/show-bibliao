var questions = [
  {
    points: 10,
    question: "Quem foi o primeiro rei de Israel",
    options: [
      { text: "Davi", correct: false },
      { text: "Salomão", correct: false },
      { text: "Saul", correct: true },
      { text: "Samuel", correct: false }
    ]
  },
  {
    points: 20,
    question: "Qual profeta profetizou o “vale de ossos secos”",
    options: [
      { text: "Isaías", correct: false },
      { text: "Jeremias", correct: false },
      { text: "Daniel", correct: false },
      { text: "Ezequiel", correct: true }
    ]
  },
  {
    points: 30,
    question: "Quem foi o profeta que substituiu Elias",
    options: [
      { text: "Eliseu", correct: true },
      { text: "Jeremias", correct: false },
      { text: "Amós", correct: false },
      { text: "Ageu", correct: false }
    ]
  },
  {
    points: 40,
    question: "Quem escreveu o livro de Atos dos apóstolos e era médico",
    options: [
      { text: "Paulo", correct: false },
      { text: "Lucas", correct: true },
      { text: "João", correct: false },
      { text: "Pedro", correct: false }
    ]
  },
  {
    points: 50,
    question: "Que profeta predisse o nascimento de Jesus em Belém",
    options: [
      { text: "Zacarias", correct: false },
      { text: "Isaías", correct: false },
      { text: "Miquéias", correct: true },
      { text: "Ageu", correct: false }
    ]
  },
  {
    points: 100,
    question: "Qual foi o discípulo que foi chamado de Dídimo",
    options: [
      { text: "Pedro", correct: false },
      { text: "Tomé", correct: true },
      { text: "Filipe", correct: false },
      { text: "Tiago", correct: false }
    ]
  },
  {
    points: 200,
    question: "Qual foi o nome do monte onde Moisés recebeu os 10 mandamentos",
    options: [
      { text: "Hermon", correct: false },
      { text: "Carmelo", correct: false },
      { text: "Sião", correct: false },
      { text: "Sinai", correct: true }
    ]
  },
  {
    points: 300,
    question: "Quantas vezes o povo de Israel rodou Jericó antes dos muros caírem",
    options: [
      { text: "7", correct: true },
      { text: "5", correct: false },
      { text: "8", correct: false },
      { text: "10", correct: false }
    ]
  },
  {
    points: 400,
    question: "Qual o nome do discípulo chamado filho do trovão",
    options: [
      { text: "Pedro", correct: false },
      { text: "Filipe", correct: false },
      { text: "André", correct: false },
      { text: "Tiago", correct: true }
    ]
  },
  {
    points: 500,
    question: "Qual era o nome original de Abraão",
    options: [
      { text: "Abias", correct: false },
      { text: "Abimeleque", correct: false },
      { text: "Abrão", correct: true },
      { text: "Abiatar", correct: false }
    ]
  },
  {
    points: 600,
    question: "Em que cidade Paulo foi preso pela primeira vez",
    options: [
      { text: "Roma", correct: false },
      { text: "Filipos", correct: true },
      { text: "Corinto", correct: false },
      { text: "Éfeso", correct: false }
    ]
  },
  {
    points: 700,
    question: "Quem escreveu a maioria das cartas do novo testamento",
    options: [
      { text: "João", correct: false },
      { text: "Pedro", correct: false },
      { text: "Tiago", correct: false },
      { text: "Paulo", correct: true }
    ]
  },
  {
    points: 800,
    question: "Quem foi a mulher que transformou sua casa num local de oração em Filipos",
    options: [
      { text: "Marta", correct: false },
      { text: "Lídia", correct: true },
      { text: "Dorcas", correct: false },
      { text: "Priscila", correct: false }
    ]
  },
  {
    points: 900,
    question: "Quem foi o primeiro sacerdote mencionado na bíblia",
    options: [
      { text: "Melquisedeque", correct: true },
      { text: "Arão", correct: false },
      { text: "Levi", correct: false },
      { text: "Moisés", correct: false }
    ]
  },
  {
    points: 1000,
    question: "Quem cortou o cabelo de Sansão",
    options: [
      { text: "Um soldado", correct: true },
      { text: "Dalila", correct: false },
      { text: "Sua mãe", correct: false },
      { text: "Um servo", correct: false }
    ]
  }
];